...
- Modify ST_FAULT:
  ...
  - **Fast recovery path**: If handshake before timeout:
    - Skip MC_Reset (ST_FAULT_RECOVERY)
    - Set `doFaultActive := FALSE`
    - Go to ST_BRAKE_HOLD (drive stays enabled, ready for op-mode handshake)
  - **Slow recovery path**: If timeout:
    - Go to ST_FAULT_IDLE (drive disabled, brake engaged)
- ST_FAULT_RECOVERY: REMOVE or repurpose (fast path skips it)
...
ST_FAULT (no brake, MC_Halt, output fault code)
  ├─ Handshake (fast) → doFaultActive := FALSE → ST_BRAKE_HOLD (ready for op-mode)
  └─ Timeout (slow) → ST_FAULT_IDLE

ST_FAULT_IDLE (brake engaged, drive off)
  └─ Handshake → ST_IDLE (full restart required)
```

**Rationale**: Fast recovery keeps drive enabled, avoiding restart delay. Timeout path is for cases where master doesn't respond quickly.
...
3. **PRG_Main.st** - Fault state architecture:
   - ST_FAULT: Remove brake, add fast path → ST_BRAKE_HOLD, timeout → ST_FAULT_IDLE
   - ST_FAULT_RECOVERY: Remove (fast path bypasses it)
   - Add ST_FAULT_IDLE state
...
- [ ] ST_FAULT fast handshake → ST_BRAKE_HOLD (drive stays enabled)
- [ ] ST_FAULT timeout → ST_FAULT_IDLE (drive disabled)
- [ ] ST_FAULT_IDLE: brake engaged, drive disabled
- [ ] ST_FAULT_IDLE handshake → ST_IDLE (full restart)
