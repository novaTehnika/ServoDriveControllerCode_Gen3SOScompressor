...
```
[[COMMENT TO LLM]]
The timeout is to allow fast recovery. If we recover from the fault with a handshake before the timeout, we should go to ST_BRAKE_HOLD instead of ST_IDLE so that we don't have to restart the drives and disengage the brake. So we need to skip the reset logic, simply set doFaultActive := FALSE and transition to ST_BRAKE_HOLD to set up for an op-mode handshake.
[[/COMMENT TO LLM]]
---
