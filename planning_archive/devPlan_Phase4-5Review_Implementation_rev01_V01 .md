...
```st
fbHalt(Execute := TRUE);           // FIRST - initiate deceleration
fbDirectControl(Enable := FALSE);  // SECOND - disable direct control
```
Add clarifying comment to code.

**Item 9: MC_Halt/MC_MoveAbsolute behavior** - Per PLCopen standard, axis remains in "Standstill" state when Execute:=FALSE. Servo maintains position. Document in code comments.
